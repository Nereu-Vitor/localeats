...
export function CartSheet() {
  ...
  return (
    <>
      ...
      {/* Gaveta do Carrinho ajustada para Viewport Dinâmica do Celular */}
      {isOpen &&
        mounted &&
        createPortal(
          <div className="fixed inset-0 z-[9998] flex justify-end bg-black/50 backdrop-blur-sm">
            <div className="w-full max-w-md bg-card p-6 shadow-xl flex flex-col h-[100dvh] max-h-[100dvh] border-l border-border animate-in slide-in-from-right duration-300">
              {/* Cabeçalho Fixo */}
              <div className="flex items-center justify-between pb-4 border-b border-border shrink-0">
                ...
                <button
                  ...
                  className="text-muted-foreground hover:text-foreground font-bold text-lg p-2"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto py-4 space-y-4 pr-1">
                {items.length === 0 ? (
                  ...
                  items.map((item: any) => (
                    <div
                      key={item.id}
                      className="flex items-center justify-between border-b border-border pb-4 gap-2"
                    >
              ...
              {/* Rodapé Fixo com Folga na Base para Celulares (pb-8) */}
              {items.length > 0 && (
                <div className="pt-4 border-t border-border space-y-2 shrink-0 mt-auto bg-card pb-8">
